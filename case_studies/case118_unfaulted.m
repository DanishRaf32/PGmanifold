...
% This script is used for nonlinear MOR of IEEE-118Bus System via linear 
...
%% Plotting
pp=20; % select max rank to plotting [1 to rmax]
r=6;%6=118 % select fixed rank for plotting outputs
plot118(tspan,rdim,E_lin,E_nonlin,err_lin,err_nonlin,CPU_time,yFOM,yROMs,yQuadROMs,xFOM,xROMs,xQuadROMs,pp,r)
err_deltar=norm(abs(yFOM(:,1) - yROMs{1,6}(:,1))); 
err_omegar=norm(abs(yFOM(:,2) - yROMs{1,6}(:,2))); 
err_Quaddeltar=norm(abs(yFOM(:,1) - yQuadROMs{1,6}(:,1))); 
err_Quadomegar=norm(abs(yFOM(:,2) - yQuadROMs{1,6}(:,2))); 
% end of file 